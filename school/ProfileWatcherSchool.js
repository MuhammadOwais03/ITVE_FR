...
const ProfileWatcherSchool = ({ navigation }) => {
  ...
  const openEditModal = (type) => {
    setEditType(type);
    setInputValue(type === 'location' ? locationName : '');
        ...
        <Modal transparent visible={modalVisible} animationType="fade">
          <BlurView intensity={90} tint="dark" style={styles.modalOverlay}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalContent}>
              <Text style={styles.modalHeader}>{editType === 'location' ? 'Update Location' : `Add ${editType}`}</Text>
              {editType === 'location' && (
                <>
...
const styles = StyleSheet.create({
  ...
  buttonGroup: { flexDirection: 'row', paddingHorizontal: 20, gap: 12, marginTop: 20 },
  actionBtn: { 
    flex: 1, 
